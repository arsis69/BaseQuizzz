...
import { getRandomFact, DID_YOU_KNOW_FACTS } from './didYouKnowFacts';
...
export default function Dashboard({ userData, onStartQuiz }: DashboardProps) {
  ...
  // Load a random fact on mount and when acknowledged facts change
  useEffect(() => {
    if (acknowledgedFactsData) {
      const acknowledgedIds = (acknowledgedFactsData as bigint[]).map(id => Number(id));
      console.log('[FACT] Acknowledged fact IDs:', acknowledgedIds);
      const randomFact = getRandomFact(acknowledgedIds);
      console.log('[FACT] Selected random fact:', randomFact.id, randomFact.fact.substring(0, 50) + '...');
      setCurrentFact(randomFact);
    } else {
      console.log('[FACT] No acknowledged facts data, showing first fact');
      setCurrentFact(DID_YOU_KNOW_FACTS[0]);
    }
  }, [acknowledgedFactsData]);
  ...
  useEffect(() => {
    if (isConfirmed) {
      ...
      const refetchWithRetries = async () => {
        ...
        // Try refetching up to 5 times with 2 second intervals
        for (let i = 0; i < 5; i++) {
          console.log(`[FACT] Refetch attempt ${i + 1}/5`);
          await refetchAcknowledgedFacts();
  ...
  }, [isConfirmed, refetchAcknowledgedFacts, refetchTotalAcknowledged]);
  ...
  const acknowledgedFactsArray = acknowledgedFactsData ? (acknowledgedFactsData as bigint[]).map(id => Number(id)) : [];
  const hasAcknowledgedCurrent = acknowledgedFactsArray.includes(currentFact.id);
